...
const runTask = async ({ taskId, abortController, trigger, firstEvent, changedFiles, }) => {
    ...
    while (true) {
        ...
        if (failedAttempts > 0) {
            ...
            const minTimeout = trigger.retry.minTimeout ?? 1000;
            const maxTimeout = trigger.retry.maxTimeout ?? 30000;
            const delay = Math.min(failedAttempts * retryFactor * minTimeout, trigger.retry.maxTimeout ?? maxTimeout);
